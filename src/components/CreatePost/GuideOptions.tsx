...
export function GuideOptions({ formData, setFormData }: GuideOptionsProps) {
  ...
  return (
    <div className="space-y-4 p-4 border-2 border-primary/20 rounded-lg bg-primary/5">
      ...
      {/* Profondeur, niveau, durée, objectif */}
      <div className="space-y-4 p-4 bg-emerald-50/60 border border-emerald-200 rounded-lg">
        <h4 className="font-semibold text-emerald-800 flex items-center gap-2 text-sm">
          📖 Profondeur & pédagogie du guide
        </h4>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            <Label htmlFor="guide_depth">Profondeur du contenu</Label>
            <Select
              value={formData.guide_depth || "standard"}
              onValueChange={(value: "light" | "standard" | "expert") =>
                setFormData({ ...formData, guide_depth: value })
              }
            >
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value="light">Léger (2 500–3 500 mots)</SelectItem>
                <SelectItem value="standard">Standard (3 500–5 000 mots)</SelectItem>
                <SelectItem value="expert">Expert (5 000–8 000 mots)</SelectItem>
              </SelectContent>
            </Select>
          </div>

          <div className="space-y-2">
            <Label htmlFor="guide_level">Niveau du lecteur</Label>
            <Select
              value={formData.guide_level || "intermediaire"}
              onValueChange={(value: "debutant" | "intermediaire" | "expert") =>
                setFormData({ ...formData, guide_level: value })
              }
            >
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            <Label htmlFor="guide_duration">Durée de lecture estimée</Label>
            <Input
              id="guide_duration"
              value={formData.guide_duration || ""}
              onChange={(e) => setFormData({ ...formData, guide_duration: e.target.value })}
              placeholder="Ex: 15 min · 30 min de lecture"
              maxLength={50}
            />
          </div>

          <div className="space-y-2">
            <Label htmlFor="guide_objective">Objectif principal du guide</Label>
            <Input
              id="guide_objective"
              value={formData.guide_objective || ""}
              onChange={(e) => setFormData({ ...formData, guide_objective: e.target.value })}
              placeholder="Ex: Réussir l'achat de panneaux solaires sans se faire avoir"
              maxLength={200}
            />
          </div>
        </div>
        ...
        <div className="space-y-2">
          <Label className="text-sm">Éléments pédagogiques à inclure</Label>
          <div className="grid grid-cols-2 gap-2">
            {[
              { key: 'tableaux', label: '📊 Tableaux comparatifs (3-5)' },
              { key: 'checklist', label: '✅ Checklist pratique' },
              { key: 'glossaire', label: '📚 Glossaire des termes' },
              { key: 'etapes', label: '🪜 Étapes pas-à-pas' },
            ].map(({ key, label }) => (
              <div key={key} className="flex items-center space-x-2 bg-white/70 px-3 py-2 rounded border">
                <Checkbox
                  id={`opt_${key}`}
                  checked={!!formData.guide_options?.[key as keyof typeof formData.guide_options]}
                  onCheckedChange={(checked) =>
                    setFormData({
                      ...formData,
                      guide_options: { ...formData.guide_options, [key]: !!checked },
                    })
                  }
                />
                <Label htmlFor={`opt_${key}`} className="cursor-pointer text-xs font-normal">
                  {label}
                </Label>
              </div>
            ))}
