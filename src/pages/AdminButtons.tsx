...
import { Star, Plus, Trash2, Edit, Save, X, ArrowLeft, Loader2 } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { useToast } from "@/hooks/use-toast";
import { ButtonEditorModal } from "@/components/RichTextEditor/ButtonEditorModal";
...
const createDefaultButtons = async (userId: string) => {
  ...
  const { error } = await supabase.from('button_presets').insert(defaultButtons);
...
const AdminButtons = () => {
  ...
  const deletePreset = async (id: string) => {
    const { error } = await supabase.from('button_presets').delete().eq('id', id);

    if (error) {
      toast({ title: "Erreur", description: error.message, variant: "destructive" });
    } else {
      loadPresets();
      toast({ title: "Supprimé", description: "Le bouton a été supprimé" });
    }
  };
  ...
  const savePresetMetadata = async (preset: ButtonPreset) => {
    const { error } = await supabase
      .from('button_presets')
      .update({ name: editingName || preset.name, description: editingDescription || preset.description })
      .eq('id', preset.id);
  ...
  const createNewPreset = async (config: any) => {
    ...
    const { error } = await supabase
      .from('button_presets')
      .insert({ user_id: user.id, name: "Nouveau bouton", description: "Description du bouton", ...convertConfigToDb(config) });
    ...
    if (error) {
      ...
      loadPresets();
      setEditorOpen(false);
      toast({ title: "Créé", description: "Le nouveau bouton a été créé" });
    }
  };
  ...
  const updatePreset = async (config: any) => {
    ...
    if (error) {
      ...
      toast({ title: "Modifié", description: "Le bouton a été mis à jour" });
    }
  };
  ...
  return (
    <>
      <Helmet>
        <title>Gestion des boutons | Prime Énergies</title>
      </Helmet>
        ...
        <main className="pt-20">
          <div className="container mx-auto px-4 py-8 max-w-6xl">
            ...
            <div className="flex items-center justify-between mb-8">
              <div>
                <h1 className="text-3xl font-bold">Gestion des boutons</h1>
                <p className="text-muted-foreground mt-1">
                  Créez et gérez vos boutons personnalisés ({presets.length}/50)
                </p>
              </div>
              <Button
                onClick={() => {
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 space-y-2">
                          {editingName === preset.id ? (
                            <div className="flex gap-2">
                              <Input
                                ...
                                className="flex-1"
                              />
                              <Button size="sm" onClick={() => savePresetMetadata(preset)}>
                            ...
                            <h3 className="font-semibold text-lg flex items-center gap-2">
                              {preset.name}
                              <Button size="sm" variant="ghost" onClick={() => setEditingName(preset.id)}>
                          ...
                          {editingDescription === preset.id ? (
                            <div className="flex gap-2">
                              ...
                              <Button size="sm" variant="ghost" onClick={() => setEditingDescription(null)}>
                                <X className="w-4 h-4" />
                              </Button>
                            </div>
                          ) : (
                            <p className="text-sm text-muted-foreground flex items-center gap-2">
                              ...
                              <Button size="sm" variant="ghost" onClick={() => setEditingDescription(preset.id)}>
                                <Edit className="w-3 h-3" />
                              </Button>
                            </p>
                          )}
                        ...
                        <div className="flex gap-2">
                          <Button
                            ...
                            className="gap-2"
                          >
                            <Star className={`w-4 h-4 ${preset.isFavorite ? 'fill-yellow-400 text-yellow-400' : ''}`} />
